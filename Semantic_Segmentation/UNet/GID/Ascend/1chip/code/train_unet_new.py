...
def run_Gid(ClassN):
    ...
    scheduler = ljnn.cosine_decay_lr(min_lr = eta_min, max_lr = initial_learning_rate, total_step = epochs * step_size,
                                     step_per_epoch = step_size, decay_epoch = 120)
    ...
    for epoch in range(epochs):
        ...
        luojianet_ms.save_checkpoint(model, f'{config.save_checkpoint_path}/unet_' + str(epoch) + '.ckpt')
        print('Save checkpoint:',f'{config.save_checkpoint_path}/unet_' + str(epoch) + '.ckpt')
