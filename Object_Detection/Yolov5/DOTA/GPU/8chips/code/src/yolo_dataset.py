...
from luojianet_ms.dataset.vision.c_transforms import HWC2CHW, Normalize
...
def create_yolo_dataset(image_dir, anno_path, batch_size, device_num, rank,
    ...
    # yolo_dataset = COCOYoloDataset(root="/media/xx/新加卷/DATA/TEST/val2017", ann_file="/media/xx/新加卷/DATA/TEST/annotations/instances_val2017.json", filter_crowd_anno=False,
    #                                    remove_images_without_annotations=False, is_training=False)
    # distributed_sampler = DistributedSampler(len(yolo_dataset), 1, 0, shuffle= True)
    # yolo_dataset.size = len(distributed_sampler)
    # hwc_to_chw = HWC2CHW()
    # config.dataset_size = len(yolo_dataset)
    # num_parallel_workers = int(1)
    ...
    #                               sampler=distributed_sampler)
    # compose_map_func = (lambda image, img_id: reshape_fn(image, img_id, config))
    # dataset1 = dataset.batch(1, drop_remainder=True)
    # data1 = next(dataset1.create_dict_iterator())
    # dataset = dataset.map(operations=compose_map_func, input_columns=["image", "img_id"],
    #                       output_columns=["image", "image_shape", "img_id"],
    #                       column_order=["image", "image_shape", "img_id"],
    #                       num_parallel_workers=8)
    ...
    # print('end')
    # yolo_dataset = COCOYoloDataset(root="/media/xx/新加卷/DATA/TEST/train2017", ann_file="/media/xx/新加卷/DATA/TEST/annotations/instances_train2017.json", filter_crowd_anno=False,
    #                                    remove_images_without_annotations=False, is_training=True)
    # first = yolo_dataset.__getitem__(0)
    ...
    # # num_parallel_workers = int(cores / device_num)
    # num_parallel_workers = int(1)
    # multi_scale_trans = MultiScaleTrans(config, 1)
    # yolo_dataset.transforms = multi_scale_trans
    # dataset_column_names = ["image", "annotation", "input_size", "mosaic_flag"]
    # output_column_names = ["image", "annotation", "bbox1", "bbox2", "bbox3",
    #                        "gt_box1", "gt_box2", "gt_box3"]
    ...
    #                               python_multiprocessing=True, num_parallel_workers=min(4, num_parallel_workers))
    # for i in range(0, 2):
    #     batch = next(dataset.create_dict_iterator())
    # dataset = dataset.map(operations=multi_scale_trans, input_columns=dataset_column_names,
    #                       output_columns=map1_out_column_names, column_order=map1_out_column_names,
    #                       num_parallel_workers=min(12, num_parallel_workers), python_multiprocessing=False)
    # for i in range(0, 2):
    #     batch = next(dataset.create_dict_iterator())
    # dataset = dataset.map(operations=PreprocessTrueBox(config), input_columns=map2_in_column_names,
    #                       output_columns=map2_out_column_names, column_order=output_column_names,
    ...
    # for i in range(0, 2):
    #     batch = next(dataset.create_dict_iterator())
    # dataset = dataset.batch(4, num_parallel_workers=min(4, num_parallel_workers), drop_remainder=True)
    # for i in range(0, 3):
