...
class DetectionBlock(nn.Module):
    ...
    def __init__(self, scale, config=None, is_training=False):
...
class YOLOV3DarkNet53(nn.Module):
    ...
    def __init__(self, config=default_config, is_training=True):
        super(YOLOV3DarkNet53, self).__init__()
        self.config = config
        self.keep_detect = self.config.keep_detect
        self.tenser_to_array = ops.TupleToArray()
        ...
        self.detect_2 = DetectionBlock('m', is_training=is_training, config=self.config)
        self.detect_3 = DetectionBlock('s', is_training=is_training, config=self.config)
        # self.detect_1 = DetectionBlock('l', config=self.config)
        # self.detect_2 = DetectionBlock('m', config=self.config)
        # self.detect_3 = DetectionBlock('s', config=self.config)
