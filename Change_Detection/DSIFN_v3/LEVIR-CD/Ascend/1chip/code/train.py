...
if __name__ == '__main__':
    ...
    train_dataset, config.steps_per_epoch = create_Dataset(config.dataset_path, config.aug, config.batch_size,
                                                           shuffle = True)
    net = DSIFN()
    ...
    lr = nn.cosine_decay_lr(min_lr = config.min_lr, max_lr = config.max_lr, total_step = config.epoch_size,
                            step_per_epoch = config.steps_per_epoch, decay_epoch = config.decay_epochs)
    ...
    mox.file.copy_parallel(config.save_checkpoint_path,
                           'obs://luojianet-benchmark/Change_Detection/DSIFN_v3/LEVIR-CD/1chip/ckpt/')
    # mox.file.copy_parallel('/cache/eval/', 'obs://luojianet-benchmark/Change_Detection/DSIFN_v2/LEVIR-CD/1chip/other/')
