...
提出了一个双任务约束的深度孪生卷积网络(DTCDSCN)模型，它包含三个子网络:一个变化检测网络和两个语义分割网络。DTCDSCN可以同时完成变化检测和语义分割，这有助于学习更具鉴别性的对象级特征，并获得完整的变化检测图。
...
1. 下载常用的遥感变化检测影像数据集，如WHU ChangeDetection等
